...
역명은 실제 출력 크기에서 서로 겹치거나 다른 역 표시를 가리지 않는 위치를 자동으로 찾습니다. `label_offsets`가 있으면 그 위치를 먼저 시도하고, 겹치면 가까운 다른 위치로 옮깁니다. 공간이 부족해 모든 역명을 놓을 수 없으면 그림 크기를 늘리라는 오류를 냅니다. 노선 교차는 자동으로 최적화하지 않으며, 노선 모양과 역의 고정 위치는 `positions`와 `anchors`로 지정합니다. 잘못된 상태 값, 중복 노선 ID, 좌표가 없는 환승역 등은 [데이터 모델](../src/schematic_transit/model.py)에서 오류로 알려줍니다.
